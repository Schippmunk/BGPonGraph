...
def app_n_proctype(i: int) -> None:
    ...
    app('byte current_minimum = max_cost;', 1)
    app('byte local_min;', 1)
    ...
    for j in range(len(succ)):
        ...
        app('if', 3)
        app('::  (paths[x].cost < local_min) -> cmi = x; local_min = paths[x].cost', 3)
        app('::  else -> true;', 3)
        app('fi;', 3)
        app('x = x + 1;', 3)
        ...
        app('if', 2)
        app('::  ((cmi_old != cmi) || (paths[cmi].cost <= current_minimum));', 2)
        app('current_minimum = local_min;', 3)
        app('cmi_old = cmi;', 3)
        for k in pred:
            if k != 0:
